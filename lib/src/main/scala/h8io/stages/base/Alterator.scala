...
/** A `h8io.stages.Stage` that wraps another stage (the ''alterand'') and forwards resource disposal to it.
  ...
  * `BaseStage` or `BaseEvolution`-style mixins). `Alterator` assumes that [[alterand]] owns all resources: `dispose` is
  * delegated to it and it alone. A co-mixed trait that adds its own resources or overrides evolution without
  * coordinating with [[alterand]] breaks this contract silently — its resources will not be released and its evolution
