...
/** A stage that immediately signals pipeline completion without dropping the input value.
  ...
  *
  * The singleton operates on `Any` and can be safely cast to any `Stage.Fruitful.Endo[T]` via `apply[T]`.
  */
object Complete extends FruitfulStaticStage.Endo[Any, Nothing] {

  /** Returns a typed view of this singleton as a `Stage.Fruitful.Endo[T]`.
    *
    * @tparam T
    *   the concrete value type
    */
  def apply[T]: Stage.Fruitful.Endo[T, Nothing] = asInstanceOf[Stage.Fruitful.Endo[T, Nothing]]
