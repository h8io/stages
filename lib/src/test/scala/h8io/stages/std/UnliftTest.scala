...
class UnliftTest extends AnyFlatSpec with Matchers with MockFactory {
  "Unlift" should "return Yield.Some for Some input" in {
    ...
    Unlift[AnyRef](Some(value)) should matchPattern { case Yield.Some(`value`, Status.Success, Unlift) => }
  }

  it should "return Yield.None for None input" in {
    Unlift[AnyRef](None) should matchPattern { case Yield.None(Status.Success, Unlift) => }
  }
}
