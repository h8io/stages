...
class ScanTest extends AnyFlatSpec with Matchers with Inside with MockFactory {
  ...
  it should "keep the accumulator when op yields no output on Success" in {
    val op1 = mock[Stage[(String, String), String, String]]("op 1")
    val opEvolution1 = mock[Evolution[(String, String), String, String]]("op evolution 1")
    val op2 = mock[Stage[(String, String), String, String]]("op 2")
    val opEvolution2 = mock[Evolution[(String, String), String, String]]("op evolution 2")
    ...
    inSequence {
      ...
      inside(Scan(op1)("a")) { case Yield.Some("a", Status.Success, evolution1) =>
        ...
        inside(stage2("b")) { case Yield.None(Status.Success, evolution2) =>
          (opEvolution2.evolve _).expects(Status.Success).returns(op3)
          val stage3 = evolution2.evolve(Status.Success)

          // Still folding against "a": the decline above must not have discarded or altered the accumulator.
          (op3.apply _).expects(("a", "c")).returns(Yield.Some("ac", Status.Success, opEvolution3))
          inside(stage3("c")) { case Yield.Some(out, Status.Success, _) =>
            out shouldBe "ac"
          }
        }
  ...
  it should "reset to a fresh Scan when a fold's status is Complete" in {
    ...
    val nextOpEvolution = mock[Evolution[(String, String), String, String]]("next op evolution")
    ...
    inSequence {
      ...
      inside(Scan(op1)("a")) { case Yield.Some("a", Status.Success, evolution1) =>
        ...
        (op2.apply _).expects(("a", "b")).returns(Yield.Some("ab", complete, opEvolution2))
  ...
  it should "reset to a fresh Scan when op declines with a Complete status" in {
    val op1 = mock[Stage[(String, String), String, String]]("op 1")
    val opEvolution1 = mock[Evolution[(String, String), String, String]]("op evolution 1")
    val op2 = mock[Stage[(String, String), String, String]]("op 2")
    val opEvolution2 = mock[Evolution[(String, String), String, String]]("op evolution 2")
    ...
    inSequence {
      ...
      inside(Scan(op1)("a")) { case Yield.Some("a", Status.Success, evolution1) =>
        ...
        (op2.apply _).expects(("a", "b")).returns(Yield.None(complete, opEvolution2))
        inside(stage2("b")) { case Yield.None(status, evolution2) =>
          status shouldBe complete
