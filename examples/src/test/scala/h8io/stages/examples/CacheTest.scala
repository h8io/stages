...
class CacheTest
  ...
  "Cache" should "cache output only if the yield is Some and the status is Success" in {
    def test(
        yieldSupplier: StatusAndOnDoneToYield[UUID, String, Exception],
      ...
      val yld = yieldSupplier(status, onDone)
      val cache = Cache(stage)
      (stage.apply _).expects(in).returns(yld)
      val cacheYield = cache(in)
      inside((yld, cacheYield)) {
        case (Yield.Some(out, status, _), Yield.Some(cacheOut, cacheStatus, cacheOnDone)) =>
        ...
        case (Yield.None(status, _), Yield.None(cacheStatus, cacheOnDone)) =>
          cacheStatus shouldBe status
    ...
    forAll(
      Gen.zip(
        Arbitrary.arbitrary[StatusAndOnDoneToYield[UUID, String, Exception]],
  ...
  "Cached" should "keep output while the status is Success" in
    forAll(Gen.zip(Gen.long, Gen.uuid)) { case (in, out) =>
      ...
      inside(cached(in)) { case Yield.Some(`out`, Status.Success, onDone) =>
