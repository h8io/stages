...
trait Stage[-I, +O, +E] extends (I => Yield[I, O, E]) {
  ...
  /** Executes this stage end-to-end and returns a plain [[Outcome]].
    ...
    *   1. Disposes the selected next stage — since `execute` is a terminal operation, the continuation is not
    *      needed and must be cleaned up immediately.
  ...
  /** Composes this stage with `that`, producing a new stage that feeds the output of this stage into `that`.
    *
    * The resulting [[Stage.AndThen]] feeds the output of this stage into `that`. If this stage produces an output,
    * the statuses and evolutions of both stages are merged; if it produces no output, only the evolutions are
    * composed and `that` is not invoked for the current input.
    *
    * @param that
