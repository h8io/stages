...
trait Evolution[-I, +O, +E] {
  ...
  /** [[map]] into an [[Evolution.Fruitful]]. Fruitfulness comes from `f` alone — this evolution need not be fruitful
    * itself — and closes over the generations by the [[Stage.Fruitful]] contract: `f` produces a fruitful stage, whose
    * own evolution is fruitful in turn. That is the shape of a decorator always emitting a value around an inner stage
    * that does not, such as `h8io.stages.operators.Lift` in the lib module.
    *
    * A separate name rather than an overload of [[map]]: two `map` alternatives taking a function make Scala 2 unable
    * to infer the parameter type of a lambda, breaking every `evolution.map(Decorator(_))` call site.
    */
  @inline final def mapToFruitful[_I, _O, _E](
      f: Stage[I, O, E] => Stage.Fruitful[_I, _O, _E]): Evolution.Fruitful[_I, _O, _E] =
    Evolution.Mapped.Fruitful(this, f)
}
...
object Evolution {
  ...
  /** An evolution every generation of which is [[Stage.Fruitful]]. */
  trait Fruitful[-I, +O, +E] extends Evolution[I, O, E] {

    override def evolve(status: Status[?]): Stage.Fruitful[I, O, E]

    /** Composes two fruitful evolutions into a fruitful one, mirroring `Stage.Fruitful.~>` for the continuations. An
      * overload, for the same reason it is one there: a mixed composition is a plain [[Evolution]].
      */
    @inline final def compose[_O, _E >: E](that: Evolution.Fruitful[O, _O, _E]): Evolution.Fruitful[I, _O, _E] =
      Evolution.AndThen.Fruitful(that, this)
  }

  object Fruitful {

    /** A fruitful evolution over stages whose input and output types coincide. */
    type Endo[T, +E] = Evolution.Fruitful[T, T, E]
  }
  ...
  def dispose(evolutions: Evolution.Any*): Unit =
    evolutions
      .foldLeft(Option.empty[Throwable]) { (primary, evolution) =>
        try {
          evolution.dispose()
          primary
        } catch {
  ...
  private[stages] object AndThen {
    ...
    /** The continuation of a fruitful composition, built by [[Evolution.Fruitful.compose]]. Its fields are named as in
      * [[Evolution.AndThen]] — see there.
      */
    final case class Fruitful[-I, OI, +O, +E](
        upstream: Evolution.Fruitful[OI, O, E],
        downstream: Evolution.Fruitful[I, OI, E])
        extends Evolution.Fruitful[I, O, E] {

      override def evolve(status: Status[?]): Stage.Fruitful[I, O, E] =
        downstream.evolve(status) ~> upstream.evolve(status)
  ...
  private[stages] object Mapped {

    /** The evolution built by [[Evolution.mapToFruitful]]. The wrapped evolution stays unconstrained: what makes every
      * generation fruitful is the result type of `f`.
      */
    final case class Fruitful[II, IO, IE, -OI, +OO, +OE](
        evolution: Evolution[II, IO, IE],
        f: Stage[II, IO, IE] => Stage.Fruitful[OI, OO, OE])
        extends Evolution.Fruitful[OI, OO, OE] {
      override def evolve(status: Status[?]): Stage.Fruitful[OI, OO, OE] = f(evolution.evolve(status))
      override def dispose(): Unit = evolution.dispose()
    }
  }
}
