...
class YieldTest
  ...
  "outcome" should "execute onDone and dispose and produce Outcome.Some for Yield.Some" in
    forAll { (yieldSupplier: OnDoneToYieldSome[Int, Array[Byte], String]) =>
      ...
      yld.outcome() shouldBe Outcome.Some(yld.out, yld.signal)
    }

  it should "execute onDone and dispose and produce Outcome.None for Yield.None" in
    forAll { (yieldSupplier: OnDoneToYieldNone[Int, Array[Byte], String]) =>
      ...
      yld.outcome() shouldBe Outcome.None(yld.signal)
    }
}
