...
- **Lifecycle contract**: for each pipeline run, exactly one of `apply(in)` or `skip()` is called on every stage.
  ...
  takes no other part in exception handling (`execute` capturing dispose failures and the `Evolution.dispose(first,
  second)` helper — the shared pairwise-disposal discipline with suppression — are the only exception-aware spots).
